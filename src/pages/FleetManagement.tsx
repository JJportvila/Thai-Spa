  ...
  return (
    <div className="space-y-8 pb-24 h-full">
      ...
      <div className={`ui-panel rounded-[24px] sm:rounded-[48px] p-5 sm:p-8 lg:p-12 text-white relative overflow-hidden shadow-2xl ${
        role === 'PLATFORM' ? 'bg-slate-900 shadow-sky-500/10' : 'bg-indigo-950 shadow-indigo-500/10'
      }`}>
         <div className={`absolute top-0 right-0 w-[500px] h-[500px] blur-[120px] rounded-full -mr-32 -mt-32 ${
           role === 'PLATFORM' ? 'bg-sky-500/20' : 'bg-indigo-500/20'
         }`} />
         ...
         <div className="flex flex-col md:flex-row justify-between items-center gap-10 relative">
            <div className="space-y-6">
               <div className="flex items-center gap-6">
                  <div className={`w-16 h-16 rounded-3xl flex items-center justify-center shadow-xl ${
                    role === 'PLATFORM' ? 'bg-sky-500 shadow-sky-500/20' : 'bg-indigo-500 shadow-indigo-500/20'
                  }`}>
                     <Truck size={32} />
                  </div>
                  <div>
               ...
               <div className="flex gap-4">
                  <div className="flex items-center gap-3 bg-white/5 py-3 px-6 rounded-2xl border border-white/10">
                    ...
                    <span className="text-xs font-black uppercase tracking-widest text-slate-300">{drivers.length} {t('registeredDrivers')}</span>
                  </div>
                  <div className="flex items-center gap-3 bg-white/5 py-3 px-6 rounded-2xl border border-white/10">
                    <CheckCircle2 className="text-emerald-400" size={16} />
                    <span className="text-xs font-black uppercase tracking-widest text-slate-300">98% {t('serviceLevel')}</span>
                  </div>
               </div>
            </div>
            ...
            <div className="min-w-[280px]">
               <button className={`ui-btn w-full py-5 rounded-[24px] sm:rounded-[32px] text-xs uppercase tracking-[0.3em] flex items-center justify-center gap-3 transition-all ${
                 role === 'PLATFORM' ? 'bg-sky-500 hover:bg-sky-600 shadow-sky-500/30' : 'bg-indigo-500 hover:bg-indigo-600 shadow-indigo-500/30'
               } text-white shadow-2xl`}>
                  <UserPlus size={20} /> {t('onboardNewDriver')}
               </button>
            </div>
         </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-6">
         <div className="ui-card flex-1 flex items-center gap-4 bg-white px-4 sm:px-8 py-4 rounded-[24px] sm:rounded-[40px] border border-slate-200 shadow-sm transition-all focus-within:border-sky-500 focus-within:shadow-xl">
            ...
            {drivers.map(driver => (
               ...
               >
                  ...
                  <div className={`absolute top-0 right-0 px-8 py-3 rounded-bl-[32px] text-[10px] font-black uppercase tracking-widest text-white ${
                    ...
                    driver.status === 'IN_TRANSIT' ? 'bg-sky-500' : 'bg-slate-400'
                  }`}>
                     {driver.status}
                  </div>
                  ...
                  <div className="flex flex-col md:flex-row gap-10">
                     <div className="space-y-8 flex-1">
                        ...
                        <div className="grid grid-cols-2 gap-6">
                           <div className="space-y-1">
                              <div className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Active Vehicle</div>
                              <div className="flex items-center gap-2 text-xs font-black text-slate-700">
                                 <Truck size={14} className="text-slate-400" /> {driver.vehicle}
                              </div>
                           </div>
                           <div className="space-y-1">
                              <div className="text-[9px] font-black text-slate-300 uppercase tracking-widest">GPS Tracker</div>
                              <div className="flex items-center gap-2 text-xs font-black text-slate-700">
                                 <MapPin size={14} className="text-sky-500" /> {driver.location}
                              </div>
                           </div>
                        </div>
                     </div>
                     ...
                     <div className="md:w-48 flex flex-col justify-center gap-4">
                        ...
                        <button className="text-[9px] font-black text-slate-300 uppercase underline text-center hover:text-rose-500 transition-colors">
                           View Legal Docs
                        </button>
                     </div>
                  </div>
                  ...
                  <div className="mt-8 pt-8 border-t border-slate-50 flex items-center justify-between">
                     <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase">
                        <Calendar size={14} /> Registered {driver.joinDate}
                     </div>
                     <div className="flex items-center gap-2 text-[10px] font-black text-sky-500 uppercase">
                        <Clock size={14} /> Last Activity 5m ago
                     </div>
                  </div>
               </motion.div>
            ))}
