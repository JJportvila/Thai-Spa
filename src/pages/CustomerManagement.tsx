    ...
    <div className="w-full space-y-4 sm:space-y-6 lg:space-y-8 pb-20 font-['Inter']">
      ...
      <div className="ui-card bg-white rounded-[24px] sm:rounded-[32px] p-4 sm:p-6 lg:p-8 border border-slate-200 shadow-sm flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 sm:gap-6 lg:gap-8">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-2xl font-['Manrope'] font-black uppercase text-[#0b193c] italic leading-none">瀹㈡埛绠＄悊绯荤粺</h1>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.2em]">CRM MODULE</span>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 w-full lg:w-auto">
          <div className="relative">
            ...
            <input 
              type="text" 
              placeholder="鎼滅储瀹㈡埛濮撳悕鎴朓D..." 
              className="pl-12 pr-6 py-4 bg-slate-50 border-none rounded-2xl text-[11px] font-bold text-[#0b193c] focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all outline-none w-full sm:w-72 lg:w-80 shadow-sm"
              value={searchQuery}
          ...
          <button 
            ...
            className="px-6 sm:px-8 py-4 bg-[#0b193c] text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-emerald-600 transition-all flex items-center justify-center gap-3 active:scale-95"
          >
            <Plus size={18} /> 鏂板瀹㈡埛
          </button>
        </div>
            ...
            {(['ALL', 'ACTIVE', 'PENDING'] as const).map(tab => (
              <button
                ...
                className={`px-6 sm:px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${
                  selectedTab === tab 
                  ? 'bg-[#0b193c] text-white shadow-xl shadow-indigo-100' 
          ...
          <div className="ui-card bg-white rounded-[24px] sm:rounded-[40px] shadow-sm border border-slate-100 overflow-hidden ring-1 ring-slate-100">
              ...
              <table className="w-full min-w-[760px] text-left">
                <thead>
                  <tr className="border-b border-slate-50 text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-50/50">
                    <th className="px-8 py-5">瀹㈡埛璇︽儏</th>
                    <th className="px-4 py-5">涓氬姟绫诲瀷</th>
                    <th className="px-4 py-5 text-right">鎬绘垚浜ら</th>
                  ...
                  {filteredCustomers.map((customer) => (
                    <tr key={customer.id} className="group hover:bg-slate-50/50 transition-all cursor-pointer">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">ID: {customer.id}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-6 text-right">
                        ...
                        <div className="text-[9px] font-bold text-emerald-500 uppercase flex items-center justify-end gap-1 mt-1">
                      ...
                      <td className="px-4 py-6 text-center">
                        <span className={`px-4 py-1.5 rounded-full text-[8px] font-black uppercase tracking-widest ${
                          ...
                          customer.status === 'PENDING' ? 'bg-amber-50 text-amber-600' :
                      ...
                      <td className="px-8 py-6 text-right">
                        <button className="p-2 text-slate-300 hover:text-[#0b193c] hover:bg-white rounded-xl transition-all"><MoreVertical size={18} /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            {filteredCustomers.length === 0 && (
              <div className="py-24 text-center">
                 ...
                 <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest italic">鏈壘鍒板尮閰嶇殑瀹㈡埛璁板綍</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="col-span-12 xl:col-span-4 space-y-4 sm:space-y-6">
          <div className="ui-panel bg-[#0b193c] p-5 sm:p-8 lg:p-10 rounded-[28px] sm:rounded-[48px] shadow-2xl text-white relative overflow-hidden ring-4 ring-slate-100">
             ...
             <div className="relative z-10 space-y-8">
                <div>
                   <p className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.3em] mb-2 italic">Global Performance</p>
                   <h2 className="text-4xl font-['Manrope'] font-black text-white italic tracking-tighter leading-none">VUV 4.5M</h2>
                   <p className="text-[10px] font-bold text-slate-400 uppercase mt-2 tracking-widest leading-none">Quarterly Wholese Volume</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                   ...
                   <div className="bg-white/5 border border-white/10 rounded-3xl p-6 hover:bg-white/10 transition-all cursor-pointer group">
                      <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Retention</p>
                      <div className="flex items-center justify-between">
                ...
                <button className="w-full py-5 bg-emerald-500 text-[#0b193c] font-black text-xs uppercase tracking-[0.2em] rounded-2xl shadow-xl hover:bg-emerald-400 transition-all flex items-center justify-center gap-3 active:scale-95">
                  <Download size={18} /> 瀵煎嚭瀹㈡埛鎶ヨ〃 (CSV)
                </button>
             </div>
          </div>
          ...
          <div className="ui-card bg-white p-5 sm:p-8 rounded-[24px] sm:rounded-[40px] shadow-sm border border-slate-100 space-y-5 sm:space-y-6">
             <h3 className="text-xs font-black text-[#0b193c] uppercase tracking-widest border-b border-slate-50 pb-4 flex items-center gap-2">
                <History size={16} className="text-slate-400" /> 杩戞湡娲昏穬
             </h3>
                ...
                {[1, 2, 3].map(i => (
                  <div key={i} className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">New Order: VUV 45,000</p>
                    </div>
                    <span className="text-[8px] font-black text-slate-300 uppercase">2M AGO</span>
              ...
              <div className="bg-[#0b193c] p-5 sm:p-8 lg:p-10 flex justify-between items-center text-white relative">
                ...
                <div className="relative z-10">
                  <h3 className="text-2xl font-['Manrope'] font-black italic uppercase tracking-tighter flex items-center gap-4">
                    <Plus size={32} className="text-emerald-400" /> 鏂板瀹㈡埛妗ｆ
                  </h3>
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 mt-1">Register new wholesaler or retail partner</p>
                </div>
              ...
              <div className="p-4 sm:p-6 lg:p-10 space-y-6 sm:space-y-8 bg-slate-50/50 overflow-y-auto max-h-[75vh]">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    <div className="space-y-2">
                       <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">鍏徃/瀹㈡埛鍚嶇О</label>
                       <input type="text" className="w-full bg-white border-2 border-slate-100 rounded-3xl px-6 py-4 text-sm font-bold text-[#0b193c] outline-none focus:border-emerald-500 shadow-sm" placeholder="e.g. Switi Vanuatu Ltd." />
                    </div>
                    <div className="space-y-2">
                       <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">涓氬姟绫诲瀷</label>
                       <select className="w-full bg-white border-2 border-slate-100 rounded-3xl px-6 py-4 text-sm font-bold text-[#0b193c] outline-none focus:border-emerald-500 shadow-sm cursor-pointer appearance-none">
                          <option>Wholesale (鎵瑰彂鍟?</option>
                          <option>Retail (闆跺敭鍟?</option>
                          <option>Corporate (浼佷笟瀹㈡埛)</option>
                       </select>
                    </div>
                 </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    <div className="space-y-2">
                       <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">鑱旂郴鐢佃瘽</label>
                       <div className="relative">
                          <Phone size={14} className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300" />
                          <input type="text" className="w-full bg-white border-2 border-slate-100 rounded-3xl pl-12 pr-6 py-4 text-sm font-bold text-[#0b193c] outline-none focus:border-emerald-500 shadow-sm" placeholder="+678 ..." />
                       </div>
                    </div>
                    <div className="space-y-2">
                       <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">鐢靛瓙閭欢</label>
                       <div className="relative">
                          <Mail size={14} className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300" />
                          <input type="email" className="w-full bg-white border-2 border-slate-100 rounded-3xl pl-12 pr-6 py-4 text-sm font-bold text-[#0b193c] outline-none focus:border-emerald-500 shadow-sm" placeholder="office@company.vu" />
                       </div>
                    </div>
                 </div>

                 <div className="space-y-2">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">缁忚惀鍦板潃</label>
                    <div className="relative">
                       <MapPin size={14} className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300" />
                       <input type="text" className="w-full bg-white border-2 border-slate-100 rounded-3xl pl-12 pr-6 py-4 text-sm font-bold text-[#0b193c] outline-none focus:border-emerald-500 shadow-sm" placeholder="Shipping Address / Main Office" />
                    </div>
                 </div>
              </div>

              <div className="p-4 sm:p-6 lg:p-10 bg-white border-t border-slate-100 flex flex-col sm:flex-row gap-3 sm:gap-4">
                 <button onClick={() => setShowAddModal(false)} className="flex-1 py-4 sm:py-5 bg-slate-100 text-slate-500 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] hover:bg-slate-200 transition-all">鍙栨秷</button>
                 <button className="ui-btn ui-btn-primary flex-1 py-4 sm:py-5 rounded-2xl text-[11px] uppercase tracking-[0.2em] shadow-xl shadow-indigo-100 px-6 sm:px-10">淇濆瓨瀹㈡埛妗ｆ</button>
              </div>
            </motion.div>
          </div>
        )}
