    ...
    <div className="w-full space-y-4 sm:space-y-6 lg:space-y-8 pb-20 font-['Inter']">
      <div className="ui-card bg-white rounded-[24px] sm:rounded-[32px] p-4 sm:p-6 lg:p-8 border border-slate-200 shadow-sm flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 sm:gap-6 lg:gap-8">
        <div className="flex items-center gap-5">
          <div className="w-14 h-14 bg-[#1a237e] rounded-2xl flex items-center justify-center text-[#1a237e] shadow-lg ring-4 ring-[#dbe7ff]">
            <Truck size={28} />
          </div>
          <div>
            <h1 className="text-2xl font-['Manrope'] font-black uppercase text-slate-900 italic leading-none">供应商管理</h1>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[10px] font-black text-[#1a237e] uppercase tracking-[0.2em]">供应商网络</span>
              <span className="w-1 h-1 bg-slate-300 rounded-full" />
              <span className="text-[10px] font-black text-[#1a237e] uppercase tracking-widest italic">瓦努阿图标准</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 w-full lg:w-auto">
          <div className="relative">
            ...
            <input
              ...
              className="pl-12 pr-6 py-4 bg-slate-50 border-none rounded-2xl text-[11px] font-bold text-slate-900 focus:ring-2 focus:ring-[#dbe7ff] focus:bg-white transition-all outline-none w-full sm:w-72 lg:w-80 shadow-sm"
              value={searchQuery}
      ...
      <div className="grid grid-cols-12 gap-4 sm:gap-6 lg:gap-8 items-start">
              ...
              <button
                ...
                className={`px-6 sm:px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${
                  selectedStatus === status
                    ? 'bg-[#1a237e] text-[#1a237e] shadow-xl shadow-slate-200'
                    ...
                    <tr key={supplier.id} className="group hover:bg-slate-50/50 transition-all cursor-pointer">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-xl bg-[#1a237e] flex items-center justify-center text-[#1a237e] font-black italic shadow-inner group-hover:bg-white transition-colors">
                            {supplier.name.slice(0, 2).toUpperCase()}
                          </div>
                          <div>
                            <div className="text-sm font-black text-slate-900 uppercase italic tracking-tight">{supplier.name}</div>
                            <div className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-1">ID: {supplier.id}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-6 text-right">
                        ...
                        <div className="text-[9px] font-bold text-slate-500 uppercase flex items-center justify-end gap-1 mt-1">最近到货：{supplier.lastDelivery}</div>
                      </td>
                      <td className="px-4 py-6 text-center">
                        <div className="flex items-center justify-center gap-1">
                          <ShieldCheck size={14} className={supplier.rating > 4 ? 'text-[#1a237e]' : 'text-[#1a237e]'} />
                          <span className="text-xs font-black text-slate-700">{supplier.rating}</span>
                        </div>
                      </td>
                      <td className="px-8 py-6 text-right">
                        <button className="p-2 text-slate-400 hover:text-[#1a237e] hover:bg-white rounded-xl transition-all">
        ...
        <div className="col-span-12 xl:col-span-4 space-y-4 sm:space-y-6">
          <div className="ui-panel bg-white p-5 sm:p-8 lg:p-10 rounded-[28px] sm:rounded-[48px] shadow-2xl text-[#1a237e] relative overflow-hidden ring-4 ring-slate-100">
            ...
            <div className="relative z-10 space-y-8">
              <div>
                <p className="text-[10px] font-black text-[#1a237e] uppercase tracking-[0.3em] mb-2 italic">供应链总值</p>
                <h2 className="text-4xl font-['Manrope'] font-black text-[#1a237e] italic tracking-tighter leading-none">VUV 15.2M</h2>
                <p className="text-[10px] font-bold text-slate-500 uppercase mt-2 tracking-widest leading-none">FY24 累计采购额</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/5 border border-white/10 rounded-3xl p-6 hover:bg-white/10 transition-all cursor-pointer group">
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">活跃合作方</p>
                  <div className="flex items-center justify-between">
                    <span className="text-2xl font-black italic">42</span>
                    <Globe size={16} className="text-[#1a237e] rotate-12" />
                  </div>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-3xl p-6 hover:bg-white/10 transition-all cursor-pointer group">
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">履约率</p>
                  <div className="flex items-center justify-between">
                    <span className="text-2xl font-black italic">92%</span>
                    <ArrowUpRight size={16} className="text-[#1a237e]" />
                  </div>
                </div>
              </div>

              <button className="w-full py-5 bg-[#1a237e] text-[#1a237e] font-black text-xs uppercase tracking-[0.2em] rounded-2xl shadow-xl hover:bg-[#1a237e] transition-all flex items-center justify-center gap-3">
          ...
          <div className="ui-card bg-white p-5 sm:p-8 rounded-[24px] sm:rounded-[40px] shadow-sm border border-slate-100 space-y-5 sm:space-y-6">
            <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest border-b border-slate-50 pb-4 flex items-center gap-2">
              <Calendar size={16} className="text-slate-500" /> 即将到货
            </h3>
            <div className="space-y-6">
              {[1, 2].map((i) => (
                <div key={i} className="flex items-center gap-4 border-l-4 border-[#dbe7ff] pl-4 py-1">
                  <div className="flex-1">
                    <p className="text-[10px] font-black text-slate-900 uppercase italic">Vanuatu Beverages</p>
                    <p className="text-[8px] font-bold text-slate-500 uppercase tracking-widest">预计到达：今天 14:00</p>
                  </div>
                  <span className="text-[8px] font-black text-[#1a237e] uppercase">追踪</span>
                </div>
              ))}
            </div>
          </div>
