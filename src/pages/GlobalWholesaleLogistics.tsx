         ...
         <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 relative">
            ...
            <div className="flex gap-4">
               <div className="p-6 bg-white/5 rounded-[32px] border border-white/10">
                  <div className="text-[10px] font-black uppercase text-slate-500 mb-1">Total Bonded Value</div>
               ...
               <div className="p-6 bg-emerald-500 rounded-[32px] text-white shadow-xl shadow-emerald-500/20">
                  <div className="text-[10px] font-black uppercase text-white/60 mb-1">Clearing Node</div>
                  <div className="text-xl font-black">Port Vila Jetty</div>
               </div>
            </div>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
         ...
         <div className="lg:col-span-2 space-y-4 sm:space-y-6">
            ...
            <div className="space-y-4">
               {shipments.map(ship => (
                 <motion.div 
                  ...
                  className="ui-card bg-white rounded-[24px] sm:rounded-[40px] p-5 sm:p-8 border border-slate-100 shadow-sm hover:shadow-2xl hover:border-indigo-100 transition-all flex flex-col md:flex-row gap-6 sm:gap-8 items-center"
                 >
                    <div className="w-20 h-20 bg-slate-50 rounded-[24px] flex items-center justify-center shrink-0 border border-slate-100 relative overflow-hidden group">
                       <Ship size={32} className="text-indigo-500 relative z-10" />
                    ...
                    <div className="flex-1 space-y-4">
                       ...
                       <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                          ...
                          <div className="space-y-1">
                             ...
                             <div className="text-xs font-black text-indigo-600">{ship.eta}</div>
                          </div>
                          <div className="space-y-1">
                             <div className="text-[9px] font-black uppercase text-slate-400">{t('status')}</div>
                             <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase ${
                                ship.status === 'SAILING' ? 'bg-indigo-50 text-indigo-500' : 'bg-emerald-50 text-emerald-500'
                             }`}>
            ...
            <div className="ui-card bg-white rounded-[24px] sm:rounded-[40px] p-5 sm:p-8 border border-slate-100 shadow-sm flex flex-col md:flex-row gap-6 sm:gap-8 items-center">
               <div className="w-20 h-20 bg-slate-50 rounded-[24px] flex items-center justify-center shrink-0 border border-slate-100 relative overflow-hidden group">
                  <Plane size={32} className="text-emerald-500 relative z-10" />
            ...
            <div className="ui-card bg-slate-50 rounded-[24px] sm:rounded-[40px] p-5 sm:p-8 lg:p-10 border border-slate-200 shadow-sm space-y-6 sm:space-y-8 h-full">
               <div className="space-y-2">
                  <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-800 flex items-center gap-2">
                     <Calculator className="text-indigo-500" /> {t('landedCostTitle')}
                  </h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase">{t('landedCostDesc')}</p>
               </div>
               ...
               <div className="space-y-6">
                  <div className="p-6 bg-white rounded-[28px] border border-slate-100 space-y-4 shadow-sm">
                     ...
                     <div className="flex justify-between items-center text-[10px] font-black uppercase text-slate-400">
                        ...
                        <span className="text-rose-500">+ {formatVT(450000)}</span>
                     </div>
                     <div className="flex justify-between items-center text-[10px] font-black uppercase text-slate-400">
                        <span>Customs Duty (Avg 15%)</span>
                        <span className="text-rose-500">+ {formatVT(1275000)}</span>
                     </div>
                     <div className="h-0.5 bg-slate-50 border-t border-dashed border-slate-200" />
                     <div className="flex justify-between items-end">
                        <div className="text-xs font-black uppercase text-slate-800">Final Landed</div>
                        <div className="text-2xl font-black text-indigo-600">{formatVT(10225000)}</div>
                     </div>
                  </div>

                  <div className="ui-panel bg-indigo-600 rounded-[24px] sm:rounded-[32px] p-5 sm:p-8 text-white relative overflow-hidden">
                     ...
                     <div className="space-y-4 relative">
                        ...
                        <div>
                           <div className="text-[10px] font-black uppercase text-white/60 tracking-widest">Pricing Strategy</div>
                           <div className="text-sm font-black mt-1">Landed cost + 25% Markup recommended for wholesalers.</div>
                        </div>
                     </div>
                  </div>
               </div>

               <div className="flex items-start gap-4 p-4 bg-amber-50 rounded-2xl border border-amber-100 mt-auto">
                  <Info className="text-amber-500 shrink-0" size={16} />
                  <p className="text-[9px] font-bold text-amber-700 uppercase leading-relaxed">
                     Exchange Rate Warning: VT/AUD down 1.2%. Adjusting Sydney shipment landing cost.
                  </p>
               </div>
            </div>
         </div>
